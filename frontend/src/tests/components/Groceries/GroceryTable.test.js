...
import GroceryTable, {showCell} from "main/components/Groceries/GroceryTable";
...
describe("GroceryTable tests", () => {
  ...
  test("Has the expected column headers, content and buttons", () => {
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <GroceryTable groceries={groceryFixtures.threeGroceries} />
        </MemoryRouter>
      </QueryClientProvider>
    );
    ...
    expect(screen.getByTestId(`${testId}-cell-row-0-col-name`)).toHaveTextContent("Banana");

    expect(screen.getByTestId(`${testId}-cell-row-1-col-id`)).toHaveTextContent("3");
    expect(screen.getByTestId(`${testId}-cell-row-1-col-name`)).toHaveTextContent("Sandwich");
  ...
  test("Has the expected column headers, content and no buttons when showButtons=false", () => {
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <GroceryTable groceries={groceryFixtures.threeGroceries} showButtons={false} />
        </MemoryRouter>
      </QueryClientProvider>
    );
  ...
  test("Edit button navigates to the edit page", async () => {
    ...
    await waitFor(() => expect(mockedNavigate).toHaveBeenCalledWith('/groceries/edit/2'));
  ...
  test("Details button navigates to the details page", async () => {
    ...
    // assert - check that the navigate function was called with the expected path
    await waitFor(() => expect(mockedNavigate).toHaveBeenCalledWith('/groceries/details/2'));
